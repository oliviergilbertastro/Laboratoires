...
dt = 1E-8  # pas d'incrémentation temporel (plus petit pour les électrons)
...
for i in range(Nelectrons):
    ...
    if i == 0:  # garde une sphère plus grosse et colorée parmis toutes les grises
        Electrons.append(simple_sphere(pos=vector(x,y,z), radius=0.03, color=color.magenta)) #, make_trail=True, retain=100, trail_radius=0.3*Ratom))
    else: Electrons.append(simple_sphere(pos=vector(x,y,z), radius=Relectron, color=gray))
...
def checkCollisions():
    ...
    r2 = 2*Relectron   # distance critique où les 2 sphères entre en contact à la limite de leur rayon
...
for k in tqdm(range(100000)):
    ...
    for i in range(Nelectrons):
        ...
        Electrons[i].pos = apos[i] = apos[i] + deltax[i]  # nouvelle position de l'atome après l'incrément de temps dt
