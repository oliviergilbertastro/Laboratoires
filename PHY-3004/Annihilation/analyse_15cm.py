...
plt.tight_layout()
plt.savefig("PHY-3004/Annihilation/Figures/theoretical_fit_15.pdf")
plt.show()
...
val_degre = res[0]/(2*np.pi)*360
inc_degre = sigmas[0]/(2*np.pi)*360

print(f"Sigma = {val_degre} \pm {inc_degre}")

fwhm = 2*np.sqrt(2*np.log(2))*val_degre
inc_fwhm = 2*np.sqrt(2*np.log(2))*inc_degre

print(f"FWHM = {fwhm} \pm {inc_fwhm}")

phi_demi = fwhm/2
inc_phi_demi = inc_fwhm/2

print(f"phi_1/2 = {phi_demi} \pm {inc_phi_demi}")
...
ax1.yaxis.set_tick_params(labelsize=15)
plt.tight_layout()
plt.savefig("PHY-3004/Annihilation/Figures/gaussian_fit_15.pdf")
plt.show()